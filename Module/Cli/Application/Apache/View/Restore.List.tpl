{terminal.color('white')}Information about: {terminal.color('light-green-blue')}<{$binary}> Apache Restore{terminal.color('reset')}
...
{for.each($nodeList as $nr => $entry)}point: {$entry.name} amount: {$entry.amount}
{/for.each}
use {terminal.color('white')}{$binary} apache restore <date> <time>{terminal.color('reset')} to restore to that point
